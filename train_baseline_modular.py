...
def save_checkpoint(model, optimizer, scheduler, epoch, old_save_name, path='/data/user/MACA-VQA/ckpts_modular/checkpoint.pth'):
    """保存模型的checkpoint"""
    state = {
...
def load_checkpoint(model, optimizer, scheduler, path='/data/user/MACA-VQA/ckpts_modular/checkpoint.pth'):
    """加载模型的checkpoint"""
...
def main(config):
    ...
    start_epoch, old_save_name = load_checkpoint(model, optimizer, scheduler,
                                  path='/data/user/MACA-VQA/ckpts_modular/checkpoint.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='ViTbCLIP_SpatialTemporalDistortion_loda')
